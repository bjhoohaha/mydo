...
#==================================== INDEX / GET ==============================
  ...
  def time_remaining
    ...
    @task_today = @active_task.where(:due_date => Date.today)
    @task_tomorrow = @active_task.where(:due_date => (Date.today + 1))
    @task_week = @active_task.where("due_date <= ? AND due_Date > ?", Date.today + 1.week, Date.today + 1)
    @task_month = @active_task.where("due_date <= ? AND due_date > ? ", Date.today + 1.month, Date.today + 1.week)
    @task_upcoming = @active_task.where("due_date > ?", Date.today + 1.month)
    @task_overdue = @active_task.where("due_date < ?", Date.today)
    @no_deadlines = @active_task.where("due_date IS NULL")
  end
